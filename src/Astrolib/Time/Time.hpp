...
template<Scale scale>
inline double operator-(const Time<scale>& first, const Time<scale>& second) noexcept {
...
template<Scale scale>
inline Time<scale> operator+(const Time<scale>& first, const double seconds) noexcept {
    double jdFormattedSecond = seconds / Time<scale>::SECONDS_PER_DAY;
    return Time<scale>{first.jd() + jdFormattedSecond};
}
template<Scale scale>
inline Time<scale> operator-(const Time<scale>& first, const double seconds) noexcept {
    double jdFormattedSecond = seconds / Time<scale>::SECONDS_PER_DAY;
